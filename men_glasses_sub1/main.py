...
# Hardcoded subcategory URLs for Men Eyewear
SUBCATEGORY_URLS = [
    "https://www.boutiqaat.com/ar-kw/men/eyewear/eyewear/l/",
    "https://www.boutiqaat.com/ar-kw/men/eyewear/optical-frames-864087313/l/",
    "https://www.boutiqaat.com/ar-kw/men/eyewear/contact-lenses-939030124/l/",
]
...
class BoutiqaatDataPipeline:
        ...
        try:
            ...
            if subcategories_data:
                excel_file = self.excel_generator.create_category_workbook(
                    category_name, subcategories_data
                )
    ...
    def _upload_product_image(self, product: Dict, category_name: str, subcategory_name: str) -> str:
        try:
            ...
            s3_path = (
                f"boutiqaat-data/year={datetime.now().strftime('%Y')}/month={datetime.now().strftime('%m')}"
                f"/day={datetime.now().strftime('%d')}/men/eyewear/images/{safe_category}"
            )
    ...
    def _upload_excel_file(self, local_path: str, category_name: str) -> bool:
        try:
            ...
            s3_path = (
                f"boutiqaat-data/year={datetime.now().strftime('%Y')}/month={datetime.now().strftime('%m')}"
                f"/day={datetime.now().strftime('%d')}/men/eyewear/excel-files"
            )
            s3_key = self.uploader.upload_local_file(local_path, s3_path, filename)
            if s3_key:
